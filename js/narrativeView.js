...
class NarrativeView extends ComponentView {
  ...
  events() {
    return {
      ...
      'swipeleft .js-narrative-controls-swipe': 'onSwipeLeft',
      'swiperight .js-narrative-controls-swipe': 'onSwipeRight'
    };
  }
  ...
  initialize(...args) {
    ...
    this.model.set({
      _textLocation: this.model.get('_textLocation') || 'aside',
      _mobileTextLocation: this.model.get('_mobileTextLocation') || 'notify'
    });
  ...
  calculateState() {
    ...
    const currentTextLocation = Adapt.device.screenSize === 'large' ? this.model.get('_textLocation') : this.model.get('_mobileTextLocation');
    this.model.set({
      _mode: mode,
      _currentTextLocation: currentTextLocation
    });
  }
  ...
  renderState() {
    ...
    this.$el.removeClass((i, className) => (className.match(/\btextlocation-\S+/g) || []).join(' '));
    this.$el.addClass(`textlocation-${this.model.get('_currentTextLocation')}`);
    this.$el.toggleClass('has-textcontrols', this.model.get('_hasNavigationInTextArea'));
  }
