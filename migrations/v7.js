...
let narratives;
console.log('narrative v7');

describe('Narrative - v7.3.1 > v7.4.0', async () => {
...
describe('Narrative - v7.4.10 > v7.4.11', async () => {
  ...
  mutateContent('Narrative - add _isStackedOnMobile attribute', async (content) => {
    narratives.forEach( item => item._isStackedOnMobile = false);
    return true;
  });
...
describe('Narrative - v7.4.13 > v7.5.0', async () => {
  ...
  mutateContent('Narrative - remove _ariaLevel override attribute', async (content) => {
    narratives.forEach( narrative => {
      const narrativeItems = narrative.items.filter((item) => item._ariaLevel)
  ...
  checkContent('Narrative - check _isStackedOnMobile attribute', async (content) => {
    const isInvalid = narratives.some(({ _ariaLevel }) => _ariaLevel);
    if (isInvalid) throw new Error('Narrative - _isStackedOnMobile is invalid');
    return true;
  });
...
describe('Narrative - v7.7.1 > v7.8.0', async () => {
